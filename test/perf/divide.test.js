...
test('test div', function() {
  ...
  for (let i = 0; i < EPOCH; i++) {
    TensorMath.divide(tensorA, tensorB, tensorX);
  }
  console.log(">>> TensorMath Divide", new Date() - now, "ms");

  now = new Date();
  for (let i = 0; i < EPOCH; i++) {
    TensorMath.divide(tensorA3D, tensorB3D, tensorX3D);
  }
  console.log(">>> TensorMath Divide on 3D", new Date() - now, "ms");
});