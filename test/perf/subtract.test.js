...
test('test subtract', function() {
  ...
  for (let i = 0; i < EPOCH; i++) {
    TensorMath.subtract(tensorA, tensorB, tensorX);
  }
  console.log(">>> TensorMath Subtract", new Date() - now, "ms");

  now = new Date();
  for (let i = 0; i < EPOCH; i++) {
    TensorMath.subtract(tensorA3D, tensorB3D, tensorX3D);
  }
  console.log(">>> TensorMath Subtract on 3D", new Date() - now, "ms");
});